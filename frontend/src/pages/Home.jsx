...
export default function Home() {
  ...
  return (
    <main className="bg-bg text-text overflow-hidden">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 text-center px-6 max-w-5xl mx-auto">
          ...
          <motion.p
            ...
            className="text-text-muted text-xl mt-8 leading-relaxed max-w-2xl mx-auto"
          >
          ...
          >
            ...
            <Link to="/programs">
              <button className="border border-gold/40 hover:border-orange-500 hover:text-orange-400 transition px-10 py-4 rounded-2xl text-lg">
                Explore Programs
              </button>
            </Link>
          </motion.div>
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-3 gap-6 text-center">
          {[
            ...
            >
              ...
              <p className="text-text-muted mt-2 text-sm uppercase tracking-wider">{s.label}</p>
            </motion.div>
          ))}
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-20 items-center">
          ...
          >
            {[
              ...
              <div key={i} className="bg-bg border border-border rounded-2xl p-6 hover:border-orange-500/40 transition">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {programs.length > 0 ? programs.map((p, i) => (
              <motion.div
                ...
                viewport={{ once: true }}
                className={`bg-surface border rounded-[28px] p-8 hover:border-orange-500/60 transition-all duration-500 hover:-translate-y-1 ${p.is_featured ? "border-orange-500/40 ring-1 ring-orange-500/20" : "border-border"}`}
              >
                ...
                <p className="text-text-muted text-sm mb-6">{p.subtitle}</p>
                    ...
                    if (pricing.length > 0) {
                      return (
                        <>
                          ...
                          <p className="text-2xl font-black text-text">{pricing[1] ? pricing[1][1] : pricing[0][1]}</p>
                        </>
                      );
                    }
                    return null;
              ...
              [
                ...
                <div key={i} className={`bg-surface border rounded-[28px] p-8 hover:border-orange-500/60 transition-all ${p.featured ? "border-orange-500/40" : "border-border"}`}>
                  ...
                  <p className="text-text-muted text-sm mb-6">{p.subtitle}</p>
                  <ul className="space-y-2 mb-8">
                    {p.features.map((f, j) => <li key={j} className="flex items-center gap-2 text-text-muted text-sm"><span className="text-orange-500 text-xs">✓</span>{f}</li>)}
                  </ul>
                  <p className="text-xs text-text-muted mb-1">Starting from</p>
                  <p className="text-2xl font-black text-text">{p.from}</p>
            ...
            {trainers.length > 0 ? (
              ...
              trainers.map((t, i) => (
                <motion.div
                  ...
                  className="cursor-pointer bg-bg border border-border rounded-3xl overflow-hidden hover:border-orange-500/40 transition-all hover:-translate-y-1 group"
                >
                  {t.image_url ? (
                  ...
                  <div className="p-7">
                    ...
                    <p className="text-text-muted mt-1 text-sm">{t.role}</p>
                    {t.bio && <p className="text-text-muted mt-3 text-sm leading-relaxed">{t.bio}</p>}
                  </div>
                </motion.div>
              ))
            ) : (
              ...
              {posts.map((post, i) => (
                ...
                >
                  {post.image_url ? (
                    ...
                    <div className={`h-48 flex items-center justify-center text-5xl ${typeColors[post.post_type] || "bg-surface-alt text-text-muted"}`}>
                      {post.post_type === "workout" ? "💪" : post.post_type === "announcement" ? "📢" : post.post_type === "video" ? "🎥" : "📸"}
                    </div>
                  )}
                  <div className="p-6">
                    <div className="flex items-center gap-2 mb-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${typeColors[post.post_type] || "bg-gray-500/20 text-text-muted"}`}>
                        {post.post_type}
                      </span>
                ...
                >
                  <div className="flex gap-1 mb-5">
                    ...
                    {[...Array(5 - r.rating)].map((_, j) => (
                      <span key={j} className="text-text-muted">★</span>
                    ))}
                  </div>
                  <p className="text-text-muted leading-8 italic mb-6">"{r.review_text || r.review}"</p>
                  <div className="border-t border-border pt-5 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {r.program && <p className="text-xs text-text-muted">{r.program}</p>}
                    </div>
                  </div>
                </motion.div>
              ))}
          ...
          >
            ...
            >
              ...
              <div className="p-8 text-center">
                ...
                <p className="text-text-muted mt-2">{lightbox.role}</p>
              </div>
            </motion.div>
          </motion.div>
        )}
